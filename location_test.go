...
func setupTestDB(t *testing.T) *ServiceOnPostgres {
	...
	// Use -V to remove volumes on down, ensuring clean state
	upCmd := exec.Command("docker", "compose", "-f", "test.docker-compose.yaml", "up", "-d", "--wait")
	err := upCmd.Run()
	// Don't fail immediately, docker might already be running or have issues, connection attempt will verify
	if err != nil {
		t.Logf("INFO: 'docker compose up' command finished (might be ignorable): %v", err)
		// Attempt to check if container is running
		...
		if psErr != nil || len(psOut) == 0 {
			t.Fatalf("Failed to start or confirm test postgres container is running: %v", err)
		}
		t.Logf("INFO: Test container seems to be running despite 'up' command warning.")
	}


	// Clean up the container when the test finishes
	t.Cleanup(func() {
		// Add -v to remove volumes for clean state next time
		cmd := exec.Command("docker", "compose", "-f", "test.docker-compose.yaml", "down", "-v")
		err := cmd.Run()
		if err != nil {
			t.Logf("WARNING: Failed to stop/clean test postgres container: %v", err)
		}
	})
	...
	for {
		...
		default:
			...
			if err == nil {
				...
				t.Logf("INFO: DB connection ping failed (will retry): %v", pingErr)
				// db.DB() // Close the potentially problematic connection - GORM handles this
			} else {
...
connected:
	// Clear existing data and run migrations for a clean slate FOR EACH TEST
	// Drop tables first
	db.Migrator().DropTable(&postgres.Relation{}, &postgres.NameMap{}, &postgres.Location{}, &postgres.GeoLevel{})
...
func TestServiceOnPostgres_RemoveAlias(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: true, // DeleteNameMap should return error if alias not found
            errType: postgres.ErrNameNotFound,
		},
        {
			name:    "remove alias that was already removed",
			geoID:   loc1.GeoID,
			alias:   "Alias1", // Removed in first test
			wantErr: true,
            errType: postgres.ErrNameNotFound,
		},
        {
        ...
        {
			name:    "remove alias from non-existent location",
			geoID:   uuid.NewString(),
			alias:   "SomeAlias",
			wantErr: true, // Should fail because location ID doesn't exist or NameMap query returns nothing
            errType: postgres.ErrNameNotFound, // Or potentially LocationNotFound depending on DeleteNameMap impl
		},
        {
			name:    "invalid geoID format",
			geoID:   "not-a-uuid",
